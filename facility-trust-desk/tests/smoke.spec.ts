...
test('smoke test - trust desk loads and ranks facilities', async ({ page }) => {
  await page.goto('/');

  // Header + filter card render immediately.
  await expect(page.getByRole('heading', { name: 'Facility Trust Desk' })).toBeVisible();
  await expect(page.getByText('Find facilities by capability')).toBeVisible();

  // Default capability (icu) returns a ranked table from the warehouse.
  await expect(page.getByRole('columnheader', { name: 'Trust' })).toBeVisible({ timeout: 30000 });
});

test('smoke test - facility drill-down shows source URLs', async ({ page }) => {
  await page.goto('/');

  // Click the first facility row to open the detail sheet.
  const firstRow = page.locator('tbody tr').first();
  await firstRow.waitFor({ state: 'visible', timeout: 30000 });
  await firstRow.click();

  await expect(page.getByText('Facility source URLs').first()).toBeVisible({ timeout: 30000 });
});
